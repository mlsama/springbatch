package com.mlsama.hellospringbatch.utils;

/**
 * Created by pangqichao on 2018/5/22.
 */
 ...
 */
@Slf4j
public class UncompressZFileUtil
{

    public static void unCompress(String compressFilePath, String uncompressFilePath)  throws Exception
    {
        FileInputStream inputStream = new FileInputStream(compressFilePath);
        InputStream in = new UncompressInputStream(inputStream);
        File file = new File(uncompressFilePath);
        if (!file.exists()){
            file.createNewFile();
        }
        ...
        log.info("Decompressed " + tot + " bytes");
        log.info("Time: " + (end-beg)/1000. + " seconds");
    }
}
