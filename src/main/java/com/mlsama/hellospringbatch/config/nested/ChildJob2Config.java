...
public class ChildJob2Config {
    ...
    public Job childJob2(){
        return jobBuilderFactory.get("childJob2Config")
                /**
                 * start是重载的方法:
                 *  1. start(Step step)
                 *  2. start(Flow flow)
                 */
        .start(childJob2ConfigStep())
        .build();
    }

    //创建Step,并交给spring管理
    @Bean
    public Step childJob2ConfigStep() {
        return stepBuilderFactory.get("childJob2ConfigStep")
        .tasklet((stepContribution,chunkContext) ->{
            log.info("********************** childJob2ConfigStep ****************");
